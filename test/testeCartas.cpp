...
TEST_CASE("31 - GetNumero"){
	CartaNormal c1 = CartaNormal(5, Cor::AZUL);

	CHECK(c1.getNumero() == 5);
}

TEST_CASE("32 - GetNumero, diferente"){
	CartaNormal c1 = CartaNormal(3, Cor::AZUL);

	CHECK_FALSE(c1.getNumero() == 5);
}

TEST_CASE("33 - GetFuncao"){
	CartaEspecial c1 = CartaEspecial(10, Cor::AZUL);

	CHECK(c1.getFuncao() == "@");
}

TEST_CASE("34 - GetFuncao, diferente"){
	CartaEspecial c1 = CartaEspecial(11, Cor::AZUL);

	CHECK_FALSE(c1.getFuncao() == "@");
}

TEST_CASE("35 - Construtor Padrão"){
	Carta c1 = Carta();
}

TEST_CASE("36 - Descricao"){
	Carta c1 = Carta();
	c1.setDescricaoCarta("carta");

	CHECK(c1.getDescricaoCarta() == "carta");
}

TEST_CASE("37 - Descricao diferente"){
	Carta c1 = Carta();
	c1.setDescricaoCarta("cartas");

	CHECK_FALSE(c1.getDescricaoCarta() == "carta");
}

// TEST_CASE("38 - CartaNaoBranca, C"){

// 	CHECK_THROWS_AS(CartaEspecial(13, Cor::AZUL), CartaNaoBranca());
// }

// TEST_CASE("39 - CartaNaoBranca, +4"){
